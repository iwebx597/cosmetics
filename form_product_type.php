...
// 📌 1. ຕອນກົດປຸ່ມບັນທຶກຂໍ້ມູນ (ເພີ່ມປະເພດສິນຄ້າໃໝ່)
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['btn_save'])) {
    $type_name = trim($_POST['type_name']);

    if (!empty($type_name)) {
        try {
            // 🔍 ກວດສອບກ່ອນວ່າ ຊື່ປະເພດສິນຄ້ານີ້ມີແລ້ວບໍ່ (ປ່ຽນເປັນ type_name)
            $check = $conn->prepare("SELECT type_name FROM product_type WHERE type_name = :type_name LIMIT 1");
            $check->execute(['type_name' => $type_name]);

            if ($check->rowCount() > 0) {
                $msg = "⚠️ ຊື່ປະເພດສິນຄ້ານີ້ ມີຢູ່ໃນລະບົບແລ້ວ!";
                $msg_type = "warning";
            } else {
                // 💾 ສັ່ງ Insert ລົງຕາຕະລາງ product_type (ປ່ຽນເປັນ type_name)
                $sql = "INSERT INTO product_type (type_name) VALUES (:type_name)";
                $stmt = $conn->prepare($sql);
                $stmt->execute(['type_name' => $type_name]);

                $msg = "ເພີ່ມປະເພດສິນຄ້າໃໝ່ສຳເລັດແລ້ວ!";
                $msg_type = "success";
            }
        ...
        $msg = "⚠️ ກະລຸນาປ້ອນຊື່ປະເພດສິນຄ້າ!";
        $msg_type = "warning";
    }
}

// 📌 2. ດຶງຂໍ້ມູນປະເພດສິນຄ້າທັງໝົດອອກມາສະແດງ
try {
    $stmt = $conn->query("SELECT * FROM product_type ORDER BY type_id asc");
...
<div class="container-fluid p-0">
    ...
    <div class="pg-header-box">
        <h4 class="pg-header-title">ຈັດການປະເພດສິນຄ້າ</h4>
    </div>
    ...
    <div class="pg-content-wrapper">
        ...
        <div class="pg-form-card">
            <div class="pg-form-card-header">
                <h6 class="pg-form-card-title">ເພີ່ມປະເພດສິນຄ້າໃໝ່</h6>
            </div>
            <div class="pg-form-body">
                <form method="POST" action="form_product_type.php">
                    <div class="mb-3 text-center">
                        <label class="form-label-custom">ຊື່ປະເພດສິນຄ້າ</label>
                        <input type="text" name="type_name" class="form-control-custom" placeholder="ຕົວຢ່າງ : ຄຣີມບຳລຸງ,ນ້ຳຫອມ" required>
                    </div>
        ...
        <!-- Table Card (List Categories) -->
        <div class="pg-form-card">
            <div class="pg-form-card-header">
                <h6 class="pg-form-card-title">ລາຍການປະເພດສິນຄ້າທັງໝົດໃນລະບົບ</h6>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="pg-table w-100" id="typeTable">
                        <thead>
                            <tr>
                            ...
                            <?php if(count($types) > 0): ?>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align:center; white-space:nowrap;">
                                        <a href="product_type_edit.php?id=<?= $type['type_id'] ?>" class="btn-edit-icon" title="ແກ້ໄຂ">
                                        ...
                                        <a href="product_type_delete.php?id=<?= $type['type_id'] ?>" class="btn-del-icon" title="ລົບ" onclick="confirmDelete(event, this.href)">
            ...
            <div class="pg-pagination-wrapper">
                <div class="pg-table-footer-info">
                    ຈຳນວນ <?= count($types) ?> ລາຍການປະເພດສິນຄ້າໃນລະບົບ
                </div>
...
function confirmDelete(event, url) {
    ...
    Swal.fire({
        title: 'ຢືນຢັນການລົບ',
        text: 'ທ່ານຕ້ອງການລົບປະເພດສິນຄ້ານີ້ແທ້ບໍ?',
        icon: 'warning',
