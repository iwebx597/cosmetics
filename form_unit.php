...
require 'cmt_db.php'; // ເcellເຊື່ອມຕໍ່ຖານຂໍ້ມູນ cmt_db

$msg = "";
$msg_type = "";

// 📌 1. ຕອນກົດປຸ່ມບັນທຶກຂໍ້ມູນ (ເພີ່ມໜ່ວຍນັບໃໝ່)
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['btn_save'])) {
    $unit_name = trim($_POST['unit_name']);

    if (!empty($unit_name)) {
        try {
            // ກວດສອບກ່ອນວ່າ ຊື່ໜ່ວຍນັບນີ້ມີແລ้ວບໍ່
            $check = $conn->prepare("SELECT unit_name FROM unit WHERE unit_name = :unit_name LIMIT 1");
            $check->execute(['unit_name' => $unit_name]);

            if ($check->rowCount() > 0) {
                $msg = "⚠️ ຊື່ໜ່ວຍນັບນີ້ ມີຢູ່ໃນລະບົບແລ້ວ!";
                $msg_type = "warning";
            } else {
                // ສັ່ງ Insert ລົງຕາຕະລາງ unit
                $sql = "INSERT INTO unit (unit_name) VALUES (:unit_name)";
                $stmt = $conn->prepare($sql);
                $stmt->execute(['unit_name' => $unit_name]);

                $msg = "ເພີ່ມໜ່ວຍນັບໃໝ່ສຳເລັດແລ້ວ!";
                $msg_type = "success";
            }
...
// 📌 2. ດຶງຂໍ້ມູນໜ່ວຍນັບທັງໝົດອອກມາສະແດງ
try {
    $stmt = $conn->query("SELECT * FROM unit ORDER BY unit_id asc");
...
<div class="container-fluid p-0">
    ...
    <div class="pg-header-box">
        <h4 class="pg-header-title">ຈັດການຫົວໜ່ວຍສິນຄ້າ</h4>
    </div>
    ...
    <div class="pg-content-wrapper">
        ...
        <div class="pg-form-card">
            <div class="pg-form-card-header">
                <h6 class="pg-form-card-title">ເພີ່ມຫົວໜ່ວຍສິນຄ້າໃໝ່</h6>
            </div>
            <div class="pg-form-body">
                <form method="POST" action="form_unit.php">
                    <div class="mb-3 text-center">
                        <label class="form-label-custom">ຊື່ຫົວໜ່ວຍສິນຄ້າ</label>
                        <input type="text" name="unit_name" class="form-control-custom" placeholder="ຕົວຢ່າງ: ກ່ອງ, ແກ້ວ, ອັນ" required>
                    </div>
        ...
        <!-- Table Card (List Units) -->
        <div class="pg-form-card">
            <div class="pg-form-card-header">
                <h6 class="pg-form-card-title">ລາຍການຫົວໜ່ວຍທັງໝົດໃນລະບົບ</h6>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="pg-table w-100" id="unitTable">
                        <thead>
                            <tr>
                            ...
                            <?php if(count($units) > 0): ?>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align:center; white-space:nowrap;">
                                        <a href="unit_edit.php?id=<?= $unit['unit_id'] ?>" class="btn-edit-icon" title="ແກ້ໄຂ">
                                        ...
                                        <a href="unit_delete.php?id=<?= $unit['unit_id'] ?>" class="btn-del-icon" title="ລົບ" onclick="confirmDelete(event, this.href)">
            ...
            <div class="pg-pagination-wrapper">
                <div class="pg-table-footer-info">
                    ຈຳນວນ <?= count($units) ?> ລາຍການຫົວໜ່ວຍໃນລະບົບ
                </div>
...
function confirmDelete(event, url) {
    ...
    Swal.fire({
        title: 'ຢືນຢັນການລົບ',
        text: 'ທ່ານຕ້ອງການລົບໜ່ວຍນັບນີ້ແທ້ບໍ? \n(ການລົບອາດສົ່ງຜົນກະທົບຕໍ່ຂໍ້ມູນສິນຄ້າທີ່ໃຊ້ໜ່ວຍນັບນີ້)',
        icon: 'warning',
